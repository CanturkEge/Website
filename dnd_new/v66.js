...
((root)=>{
  'use strict';
  let orders=[],ordersCampaign='',ordersLoading=false;
  ...
  const orderPrice=o=>o.status==='countered'?o.dm_offer:o.player_offer;
  ...
  function focusKind(item){if(['arcane','divine','druidic','instrument','universal'].includes(item?.focusType))return item.focusType;const text=`${item?.name||''} ${item?.effect||''} ${item?.note||''}`.toLocaleLowerCase('tr-TR');if(/kutsal sembol|kutsal odak|holy symbol/.test(text))return 'divine';if(/druid odağı|druidic focus/.test(text))return 'druidic';if(/lavta|çalgı|instrument/.test(text))return 'instrument';if(/arcane focus|component pouch|bileşen (kesesi|çantası)/.test(text))return 'arcane';if(item?.category==='focus'||item?.slot==='focus'||/büyü odağı/.test(text))return classFocus(item);return ''}
  ...
  exEnsureState=function(){oldEnsure();if(current?.role!=='dm')return;let changed=false;state.market=Array.isArray(state.market)?state.market:[];if((+state.shopSeedVersion||0)<8){for(const item of V66_CASTER_MARKET)if(!state.market.some(x=>x.id===item.id||x.name===item.name)){state.market.push({...item});changed=true}state.shopSeedVersion=8;changed=true}if((+state.focusSchemaVersion||0)<1){for(const item of state.market)changed=upgradeFocus(item)||changed;for(const c of state.characters||[])for(const item of c.inventory||[])changed=upgradeFocus(item)||changed;state.focusSchemaVersion=1;changed=true}if(changed)setTimeout(save,0)};
  ...
  async function loadOrders(force=false){
    ...
    if(current?.id!==campaign)return;if(error){orders=[];ordersCampaign='error';return}orders=data||[];ordersCampaign=campaign;if(page==='market')render();
  }
  ...
  iaItem=function(item,index,actions=false){let html=oldIaItem(item,index,actions);if(actions&&focusKind(item))html=html.replace(item.equipped?'Çıkar':'Kuşan',item.equipped?'Odağı Bırak':'Odak Olarak Kullan');if(actions&&current?.role==='player'&&(state.npcs||[]).length)html=html.replace('<button class="danger" data-v25-trash',`<button class="ghost" data-v66-npc-item="${index}">NPC’ye Ver</button><button class="danger" data-v25-trash`);return html};
