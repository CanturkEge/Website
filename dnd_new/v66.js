...
((root)=>{
  ...
  const clearCartOffer=()=>{const field=$('#v66CartOffer');if(field)window.kadimUiState?.clearDirty(field)};
  ...
  function orderCard(o){
    ...
    return `<article class="v66-order ${active?'active':'closed'}" data-entity-id="${esc(o.id)}"><header><span><b>${esc(o.player_name)}</b><small>${esc(orderStatus[o.status]||o.status)} • ${timeLabel(o.updated_at)}</small></span><span class="v69-current-price"><small>Güncel teklif</small><b>${money(orderPrice(o))}</b></span></header>${o.negotiation_locked&&active?'<div class="v69-final-price">🔒 DM son fiyatı — pazarlığa kapalı</div>':''}<ul class="v69-order-items">${(o.items||[]).map(i=>{const qty=Math.max(1,+i.qty||1),unit=Math.max(0,+i.unitPrice||0);return `<li><span>${esc(i.name)} ×${qty}</span><small>Normal: ${money(unit*qty)}${qty>1?` • adet ${money(unit)}`:''}</small></li>`}).join('')}</ul><div class="v69-value-line"><span>Normal toplam <b>${money(o.list_total)}</b></span><span>Son oyuncu teklifi <b>${money(o.player_offer)}</b></span>${o.dm_offer!=null?`<span>Son DM teklifi <b>${money(o.dm_offer)}</b></span>`:''}</div>${history.length>1?offerHistory(o):''}<div class="v66-order-actions">${dmTurn?`<button class="primary" data-v66-order-approve="${o.id}">Oyuncu Teklifini Kabul Et</button><div class="v69-counter-box"><label>DM karşı teklifi (CP)<input class="v69-number" data-ui-key="order:${esc(o.id)}:dm-amount" data-v69-dm-amount="${o.id}" type="number" min="1" step="1" value="${esc(String(Math.max(1,orderPrice(o))))}" inputmode="numeric"></label><label class="v69-lock"><input data-ui-key="order:${esc(o.id)}:dm-lock" data-v69-dm-lock="${o.id}" type="checkbox"> Son fiyat — oyuncu geri teklif veremesin</label><button class="ghost" data-v69-dm-counter="${o.id}">Karşı Teklif Gönder</button></div><button class="danger" data-v66-order-reject="${o.id}">Reddet</button>`:''}${isDm&&o.status==='countered'?'<span class="v69-waiting">Oyuncunun yanıtı bekleniyor.</span>':''}${playerTurn?`<button class="primary" data-v66-order-accept="${o.id}">${o.negotiation_locked?'Son Fiyatı Kabul Et':'DM Teklifini Kabul Et'}</button>${o.negotiation_locked?'':`<div class="v69-counter-box"><label>Yeni teklifin (CP)<input class="v69-number" data-ui-key="order:${esc(o.id)}:player-amount" data-v69-player-amount="${o.id}" type="number" min="1" step="1" value="${esc(String(Math.max(1,+o.player_offer||1)))}" inputmode="numeric"></label><button class="ghost" data-v69-player-counter="${o.id}">Karşı Teklif Gönder</button></div>`}`:''}${!isDm&&active?`<button class="danger" data-v66-order-cancel="${o.id}">İptal Et</button>`:''}</div></article>`;
  }
  ...
  async function finishOrder(id,action){if(!auth?.sessionToken)return alert('Güvenli market oturumu bulunamadı. Çıkış yapıp yeniden giriş yap.');let error;try{({error}=await db.rpc('market_order_finish_v66',{p_session_token:auth.sessionToken,p_campaign:current.id,p_order:id,p_action:action}))}catch(failure){error={message:failure?.message||'Market bağlantısı kurulamadı'}}if(error)return alert(error.message);window.kadimUiState?.clearWithin(document.querySelector(`[data-entity-id="${id}"]`));ordersCampaign='';auditCampaign='';exWalletCampaign=null;await Promise.all([loadOrders(true),syncFromServer(false),exLoadWallets(true)]);render();toast('Teklif işlemi tamamlandı')}
  document.addEventListener('click',async event=>{
    const b=event.target.closest('button');if(!b||!current)return;
    if(b.dataset.v66CartAdd){event.preventDefault();event.stopImmediatePropagation();const item=state.market.find(x=>x.id===b.dataset.v66CartAdd),cart=readCart(),row=cart.find(x=>x.itemId===item?.id);if(!item)return;if(row)row.qty=Math.min(item.stock,row.qty+1);else cart.push({itemId:item.id,qty:1});writeCart(cart);clearCartOffer();render();toast(item.name+' sepete eklendi');return}
    for(const [key,delta] of [['v66CartPlus',1],['v66CartMinus',-1]])if(b.dataset[key]){const cart=readCart(),row=cart.find(x=>x.itemId===b.dataset[key]);if(row)row.qty+=delta;writeCart(cart.filter(x=>x.qty>0));clearCartOffer();render();return}
    if(b.dataset.v66CartRemove){writeCart(readCart().filter(x=>x.itemId!==b.dataset.v66CartRemove));clearCartOffer();render();return}
    if(b.id==='v66ClearCart'){writeCart([]);clearCartOffer();render();return}
    if(b.id==='v66SubmitCart'){const rows=cartRows(),offer=Math.trunc(+$('#v66CartOffer')?.value||0);if(!rows.length)return;if(!auth?.sessionToken)return alert('Güvenli market oturumu bulunamadı. Çıkış yapıp yeniden giriş yap.');b.disabled=true;let error;try{({error}=await db.rpc('market_order_submit_v69',{p_session_token:auth.sessionToken,p_campaign:current.id,p_items:rows.map(r=>({itemId:r.itemId,qty:r.qty})),p_offer:offer}))}catch(failure){error={message:failure?.message||'Market bağlantısı kurulamadı'}}if(error){b.disabled=false;return alert('Teklif gönderilemedi: '+error.message+'\n\nv69-update.sql dosyasını Supabase’te çalıştır.')};window.kadimUiState?.clearWithin(b.closest('.v66-cart'));writeCart([]);ordersCampaign='';await loadOrders(true);render();toast('Teklif DM’e gönderildi');return}
    if(b.id==='v66RefreshOrders'){ordersCampaign='';await loadOrders(true);return}
    if(b.dataset.v69DmCounter){const id=b.dataset.v69DmCounter,amount=Math.trunc(+document.querySelector(`[data-v69-dm-amount="${id}"]`)?.value||0),locked=!!document.querySelector(`[data-v69-dm-lock="${id}"]`)?.checked;if(amount<1)return alert('Geçerli bir karşı teklif gir.');b.disabled=true;const {error}=await db.rpc('market_order_counter_v69',{p_session_token:auth.sessionToken,p_campaign:current.id,p_order:id,p_amount:amount,p_locked:locked});if(error){b.disabled=false;return alert(error.message)}window.kadimUiState?.clearWithin(b.closest('.v66-order'));ordersCampaign='';await loadOrders(true);toast(locked?'Son fiyat gönderildi':'Karşı teklif gönderildi');return}
    if(b.dataset.v69PlayerCounter){const id=b.dataset.v69PlayerCounter,amount=Math.trunc(+document.querySelector(`[data-v69-player-amount="${id}"]`)?.value||0);if(amount<1)return alert('Geçerli bir karşı teklif gir.');b.disabled=true;const {error}=await db.rpc('market_order_player_counter_v69',{p_session_token:auth.sessionToken,p_campaign:current.id,p_order:id,p_amount:amount});if(error){b.disabled=false;return alert(error.message)}window.kadimUiState?.clearWithin(b.closest('.v66-order'));ordersCampaign='';await loadOrders(true);toast('Yeni teklifin DM’e gönderildi');return}
