...
((root)=>{
  ...
  async function loadOrders(force=false){
    ...
    try{({data,error}=await db.rpc('market_order_list_v66',{p_session_token:auth.sessionToken,p_campaign:campaign}))}catch(failure){error={message:failure?.message||'Market bağlantısı kurulamadı'}}
    ordersLoading=false;
  ...
  document.addEventListener('click',async event=>{
    ...
    if(b.id==='v66SubmitCart'){const rows=cartRows(),offer=Math.trunc(+$('#v66CartOffer')?.value||0);if(!rows.length)return;if(!auth?.sessionToken)return alert('Güvenli market oturumu bulunamadı. Çıkış yapıp yeniden giriş yap.');b.disabled=true;let error;try{({error}=await db.rpc('market_order_submit_v66',{p_session_token:auth.sessionToken,p_campaign:current.id,p_items:rows.map(r=>({itemId:r.itemId,qty:r.qty})),p_offer:offer}))}catch(failure){error={message:failure?.message||'Market bağlantısı kurulamadı'}}if(error){b.disabled=false;return alert('Teklif gönderilemedi: '+error.message)}writeCart([]);ordersCampaign='';await loadOrders(true);render();toast('Teklif DM’e gönderildi ve DM bildirimi oluşturuldu');return}
    if(b.id==='v66RefreshOrders'){ordersCampaign='';await loadOrders(true);return}
    if(b.dataset.v66OrderCounter){const amount=Math.trunc(+(prompt('Karşı teklif (CP)',String(b.dataset.max))||0));if(!amount)return;if(!auth?.sessionToken)return alert('Güvenli market oturumu bulunamadı. Çıkış yapıp yeniden giriş yap.');let error;try{({error}=await db.rpc('market_order_counter_v66',{p_session_token:auth.sessionToken,p_campaign:current.id,p_order:b.dataset.v66OrderCounter,p_amount:amount}))}catch(failure){error={message:failure?.message||'Market bağlantısı kurulamadı'}}if(error)return alert(error.message);ordersCampaign='';await loadOrders(true);toast('Karşı teklif gönderildi');return}
