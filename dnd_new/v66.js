/* v66-v70: focus readiness, stable negotiated market cart, audit log and player-to-NPC transfers. */
((root)=>{
  'use strict';
  let orders=[],ordersCampaign='',ordersLoading=false,ordersError='';
  let marketRenderPending=false,cartOfferDraft=null;
  ...
  const marketEditing=()=>page==='market'&&!!document.activeElement?.matches?.('#v66CartOffer,[data-v69-dm-amount],[data-v69-player-amount],[data-v69-dm-lock]');
  function orderDraft(o){
    const turn=`${current?.id||''}:${current?.role||''}:${o.status}:${o.last_offer_by||'player'}:${o.updated_at||''}`;
    let draft=orderDrafts.get(o.id);
    if(!draft||draft.turn!==turn){draft={turn,dmAmount:String(Math.max(1,orderPrice(o))),playerAmount:String(Math.max(1,+o.player_offer||1)),locked:false};orderDrafts.set(o.id,draft)}
    return draft;
  }
  function resetCartOffer(){cartOfferDraft=null}
  function cartOfferValue(total){return cartOfferDraft?.key===cartKey()?cartOfferDraft.value:String(total)}
  ...
  async function loadOrders(force=false){
    ...
    ordersLoading=false;
    if(current?.id!==campaign)return;if(error){orders=[];ordersError=error.message||'Teklifler alınamadı';ordersCampaign=campaign;if(page==='market')render();return}
    const next=data||[],changed=ordersCampaign!==campaign||ordersError||JSON.stringify(next)!==JSON.stringify(orders);
    orders=next;ordersError='';ordersCampaign=campaign;
    const ids=new Set(orders.map(o=>o.id));for(const id of orderDrafts.keys())if(!ids.has(id))orderDrafts.delete(id);
    if(page==='market'&&changed)render();
  }
  ...
  render=function(){if(marketEditing()){marketRenderPending=true;return}marketRenderPending=false;return stableRender()};
  function rememberMarketDraft(target){
    if(target.id==='v66CartOffer'){cartOfferDraft={key:cartKey(),value:target.value};return}
    const id=target.dataset?.v69DmAmount||target.dataset?.v69PlayerAmount||target.dataset?.v69DmLock;if(!id)return;
    const order=orders.find(o=>o.id===id);if(!order)return;const draft=orderDraft(order);
    if(target.dataset.v69DmAmount)draft.dmAmount=target.value;
    if(target.dataset.v69PlayerAmount)draft.playerAmount=target.value;
    if(target.dataset.v69DmLock)draft.locked=target.checked;
  }
  document.addEventListener('input',event=>rememberMarketDraft(event.target),true);
  document.addEventListener('change',event=>rememberMarketDraft(event.target),true);
  document.addEventListener('focusout',()=>setTimeout(()=>{if(marketRenderPending&&!marketEditing()&&page==='market')render()},0),true);
  ...
  async function finishOrder(id,action){if(!auth?.sessionToken)return alert('Güvenli market oturumu bulunamadı. Çıkış yapıp yeniden giriş yap.');let error;try{({error}=await db.rpc('market_order_finish_v66',{p_session_token:auth.sessionToken,p_campaign:current.id,p_order:id,p_action:action}))}catch(failure){error={message:failure?.message||'Market bağlantısı kurulamadı'}}if(error)return alert(error.message);orderDrafts.delete(id);ordersCampaign='';auditCampaign='';exWalletCampaign=null;await Promise.all([loadOrders(true),syncFromServer(false),exLoadWallets(true)]);render();toast('Teklif işlemi tamamlandı')}
  document.addEventListener('click',async event=>{
    const b=event.target.closest('button');if(!b||!current)return;
    if(b.dataset.v66CartAdd){event.preventDefault();event.stopImmediatePropagation();const item=state.market.find(x=>x.id===b.dataset.v66CartAdd),cart=readCart(),row=cart.find(x=>x.itemId===item?.id);if(!item)return;if(row)row.qty=Math.min(item.stock,row.qty+1);else cart.push({itemId:item.id,qty:1});writeCart(cart);resetCartOffer();render();toast(item.name+' sepete eklendi');return}
    for(const [key,delta] of [['v66CartPlus',1],['v66CartMinus',-1]])if(b.dataset[key]){const cart=readCart(),row=cart.find(x=>x.itemId===b.dataset[key]);if(row)row.qty+=delta;writeCart(cart.filter(x=>x.qty>0));resetCartOffer();render();return}
    if(b.dataset.v66CartRemove){writeCart(readCart().filter(x=>x.itemId!==b.dataset.v66CartRemove));resetCartOffer();render();return}
    if(b.id==='v66ClearCart'){writeCart([]);resetCartOffer();render();return}
    if(b.id==='v66SubmitCart'){const rows=cartRows(),offer=Math.trunc(+$('#v66CartOffer')?.value||0);if(!rows.length)return;if(!auth?.sessionToken)return alert('Güvenli market oturumu bulunamadı. Çıkış yapıp yeniden giriş yap.');b.disabled=true;let error;try{({error}=await db.rpc('market_order_submit_v69',{p_session_token:auth.sessionToken,p_campaign:current.id,p_items:rows.map(r=>({itemId:r.itemId,qty:r.qty})),p_offer:offer}))}catch(failure){error={message:failure?.message||'Market bağlantısı kurulamadı'}}if(error){b.disabled=false;return alert('Teklif gönderilemedi: '+error.message+'\n\nv69-update.sql dosyasını Supabase’te çalıştır.')};writeCart([]);resetCartOffer();ordersCampaign='';await loadOrders(true);render();toast('Teklif DM’e gönderildi');return}
    if(b.id==='v66RefreshOrders'){ordersCampaign='';await loadOrders(true);return}
    if(b.dataset.v69DmCounter){const id=b.dataset.v69DmCounter,amount=Math.trunc(+document.querySelector(`[data-v69-dm-amount="${id}"]`)?.value||0),locked=!!document.querySelector(`[data-v69-dm-lock="${id}"]`)?.checked;if(amount<1)return alert('Geçerli bir karşı teklif gir.');b.disabled=true;const {error}=await db.rpc('market_order_counter_v69',{p_session_token:auth.sessionToken,p_campaign:current.id,p_order:id,p_amount:amount,p_locked:locked});if(error){b.disabled=false;return alert(error.message)}orderDrafts.delete(id);ordersCampaign='';await loadOrders(true);toast(locked?'Son fiyat gönderildi':'Karşı teklif gönderildi');return}
    if(b.dataset.v69PlayerCounter){const id=b.dataset.v69PlayerCounter,amount=Math.trunc(+document.querySelector(`[data-v69-player-amount="${id}"]`)?.value||0);if(amount<1)return alert('Geçerli bir karşı teklif gir.');b.disabled=true;const {error}=await db.rpc('market_order_player_counter_v69',{p_session_token:auth.sessionToken,p_campaign:current.id,p_order:id,p_amount:amount});if(error){b.disabled=false;return alert(error.message)}orderDrafts.delete(id);ordersCampaign='';await loadOrders(true);toast('Yeni teklifin DM’e gönderildi');return}
    if(b.id==='v70ClearOrderHistory'){const own=current.role!=='dm';if(!confirm(own?'Tamamlanmış, reddedilmiş ve iptal edilmiş kendi market kayıtların temizlensin mi?':'Kampanyadaki tamamlanmış, reddedilmiş ve iptal edilmiş tüm market kayıtları temizlensin mi?'))return;b.disabled=true;const {data,error}=await db.rpc('market_order_clear_history_v70',{p_session_token:auth.sessionToken,p_campaign:current.id});if(error){b.disabled=false;return alert(error.message+'\n\nv70-update.sql dosyasını Supabase’te çalıştır.')};ordersCampaign='';await loadOrders(true);toast(`${data||0} geçmiş kayıt temizlendi`);return}
