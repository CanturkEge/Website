...
async function adminRequest(action,extra={}){
 const response=await fetch(`${cfg.SUPABASE_URL}/functions/v1/kadim-admin`,{
  method:'POST',headers:{apikey:cfg.SUPABASE_ANON_KEY,'Content-Type':'application/json'},
  body:JSON.stringify({action,username:'admin',password:kadimAdminPassword,...extra})
 });
 const payload=await response.json().catch(()=>({error:'Admin sunucusu geçersiz cevap verdi'}));
 if(!response.ok)throw Error(payload.error||'Admin işlemi reddedildi');
 return payload.data;
}
...
async function loadAdminCampaigns(){
 ...
 let rows;
 try{rows=await adminRequest('list')}catch(error){if(area)area.innerHTML=`<div class="admin-error">${esc(error.message)}<br><small>Admin Edge Function yapılandırmasını kontrol et.</small></div>`;return}
 rows=rows||[];const members=rows.reduce((n,x)=>n+(+x.member_count||0),0),characters=rows.reduce((n,x)=>n+(+x.character_count||0),0);
...
document.querySelector('#authSubmit').onclick=async()=>{
 ...
 if(!registerMode&&username.toLowerCase()==='admin'){
  ...
  kadimAdminPassword=password;
  try{await adminRequest('login')}catch(error){kadimAdminPassword='';document.querySelector('#authMsg').textContent=error.message||'Admin kullanıcı adı veya şifre yanlış';return}
