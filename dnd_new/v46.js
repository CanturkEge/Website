...
(()=>{
  ...
  function v46ItemCard(item,index,actions=false){
    let group=v46Group(v46ItemGroup(item)),slot=v46Slot(item),slotMeta=v46SlotMeta(slot),bonuses=v46ItemBonuses(item),detail=[item.note,item.effect].filter(Boolean).join(' • '),summary=v46ItemBrief(item),quantity=Math.max(1,+item.qty||1),title=`${group.label}${slot?` • ${slotMeta.label}`:''}`;
