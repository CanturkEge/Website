...
const V37_PATCH_NOTES=[
  ...
  },
  {
    ...
    fixed:['XP alanı olmayan eski karakterler mevcut seviyelerinin taban XP’sine güvenli taşınır; kimse kendiliğinden 1. seviyeye düşmez.','XP eşiği geçildiğinde seviye ve seviyeye bağlı otomatik HP/stat/AC hesapları birlikte yenilenir.','Sürüm Notları üst özeti, sürüm aralığı ve varsayılan açık kart artık en yeni sürüm kaydından otomatik belirlenir.'],
  ...
  },
  {
    ...
    fixed:['Bahis, ödeme ve iptal iadesi aynı veritabanı işlemi içinde Kesem bakiyesine uygulanır.','Tekrar gönderilen aynı bahis ikinci kez para çekmez; yetersiz bakiye hiçbir kısmi kayıt bırakmaz.','Açık çok oyunculu masanın oranları masa kapanmadan değiştirilemez.'],
    changed:['Kumarhane varsayılan olarak kapalı gelir; yalnız DM açabilir.','Bahis hareketleri bildirim üretmez, DM İşlem Logu’na sessizce yazılır.','Yeni SQL tabloları kampanya JSON’u, hesaplar, karakterler, market ve eski cüzdan kayıtlarını değiştirmez.']
  },
  {
    version:'3.6.1',build:'Build 76',title:'Güvenli Yedek Geri Yükleme',tag:'YEDEK',tone:'current',
    summary:'DM, indirilen kampanya JSON’unu doğrulama, içerik özeti ve otomatik geri dönüş yedeğiyle yeniden içe aktarabilir.',
    added:['DM’ye özel Yedeği içe aktar düğmesi ve JSON dosya seçici.','Mevcut ve yedekteki karakter, NPC, eşya, ilişki, görev, savaşçı ve market sayılarını gösteren önizleme.','Geri yüklemeden hemen önce mevcut kampanya state’ini otomatik indiren güvenlik yedeği.'],
    fixed:['Yedek al ile indirilen eski düz JSON dosyaları artık arayüzden geri yüklenebilir.','Bozuk, ilgisiz, 10 MB üstü, yanlış alan tipli veya güvenli olmayan JSON kayıt çağrısından önce reddedilir.','İçe aktarma yalnız DM’de görünür ve mevcut v31 Supabase kayıt/birleştirme hattını kullanır.'],
    changed:['Yedek geri yükleme yalnız kampanya JSON state’ini değiştirir; hesaplar, üyeler ve ayrı SQL geçmiş tabloları korunur.','Yeni SQL veya veri taşıması gerekmez.']
  },
  {
  ...
  },
  {
    ...
    fixed:['Artificer 1. seviye slotları ortak kurala bağlandı.','Mobil menünün savaş paleti ve token detay paneliyle üst üste binmesi giderildi.','Tekrar gönderilen kullanım veya teslim isteği iki kez slot, para ya da eşya harcamaz.','Belge ve atış verileri hesap/kampanya değişiminde temizlenir; form taslakları arka plan yenilemelerinde korunur.'],
    changed:['DM slot tablosu, oyuncu savaş araçları ve dinlenme aynı slot alanını paylaşır; Warlock pact ve Mystic Arcanum ayrıdır.','Hasar/şifa, isabet, aksiyon ekonomisi ve pahalı bileşen tüketimi DM tarafından çözümlenir; kullanım kaydı bunları kendiliğinden uygulamaz.','Yeni ekranlar telefon için tek sütun, okunur alanlar ve geniş dokunma hedefleri kullanır.','Yeni RPC ve tetikleyiciler v74-update.sql içindedir. Hesap, karakter ve mevcut envanter kimlikleri korunur.']
  },
  {
  ...
  },
  {
    ...
    fixed:['İç içe main boşluğu kaldırıldı; tahta, kareler, tokenlar, objeler ve sis birlikte ölçekleniyor.','Seçim yapıldığında sol ray detay paneline otomatik kayıyor.'],
    changed:['Geniş ekranda sol yönetim rayı + büyüyen tahta, dar ekranda tahta öncelikli tek kolon kullanılır.','Supabase state, realtime ve oyuncu hareket yetkileri değiştirilmedi.']
  },
  {
  ...
  },
  {
    ...
    fixed:['Eski kutsal sembol, arcane focus ve component pouch kayıtları veri kaybetmeden kullanılabilir odaklara yükseltilir.','Para, stok ve envanter yalnız sunucudaki son onay işleminde birlikte güncellenir.'],
    changed:['Karşı teklif verildiyse son kabul oyuncuya aittir; DM bekleyen ilk teklifi doğrudan kabul edebilir.','Mevcut hesap, kampanya, karakter, NPC ve envanter kayıtları korunur.']
  },
  {
  ...
  },
  {
    ...
    fixed:['“Kıdemli Yarasa” gibi kör rütbe kopyaları kaldırıldı.','Kale marketlerinde Büyücü Dükkânı tierlerinin hiç açılmaması giderildi.'],
  ...
  },
  {
    ...
    fixed:[],changed:['Ganimet parası doğrudan karakter cüzdanına gönderilebilir.','Savaş araçlarının görsel hiyerarşisi sadeleştirildi.']
  },
  {
...
function v37CompareVersions(a,b){
  let aa=String(a).split('.').map(Number),bb=String(b).split('.').map(Number),length=Math.max(aa.length,bb.length);
  for(let i=0;i<length;i++){let diff=(aa[i]||0)-(bb[i]||0);if(diff)return diff}
  return 0;
}

function v37LatestNote(){
  return V37_PATCH_NOTES.reduce((latest,note)=>v37CompareVersions(note.version,latest.version)>0?note:latest,V37_PATCH_NOTES[0]);
}
...
function v37PatchRows(){
  ...
  return rows.slice().sort((a,b)=>v37PatchOrder==='asc'?v37CompareVersions(a.version,b.version):v37CompareVersions(b.version,a.version));
}
...
function v37PatchPage(){
  ...
  <section class="v37-patch-page">
    <div class="v37-patch-hero">
      <div><span class="v26-kicker">KADİM MASA DEFTERİ</span><h2>v${esc(latest.version)} • ${esc(latest.build)}</h2><p>${esc(latest.summary)}</p></div>
      <div class="v37-patch-stats"><span><b>${V37_PATCH_NOTES.length}</b>Sürüm</span><span><b>6</b>Şans Oyunu</span><span><b>Atomik</b>Bahis</span><span><b>24</b>Boss</span><span><b>319</b>Büyü</span></div>
    </div>
    ...
    <p class="v37-version-note">v0.1.0–v${esc(latest.version)} oyuncuya açık kilometre taşı numaralarıdır. Düzeltme patch'i, yeni sistem minor'ı, büyük deneyim değişimi major'ı artırır; segmentler 9’dan sonra 10 diye devam edebilir. “Build” etiketi dağıtılan teknik pakettir.</p>
