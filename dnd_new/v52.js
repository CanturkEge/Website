...
(()=>{
  ...
  function v52ClericPanel(character){
    ...
    const level=v52Level(character.level),stats=prStats(character),wis=prMod(stats.WIS),prof=prProf(level),saveDc=8+prof+wis,spellAttack=prof+wis,prepared=Math.max(1,level+wis),slots=(window.V52_CLERIC_SPELL_SLOTS||[])[level]||[],domain=v52Domain(character.subclass),deity=v52Deity(character.deityId),domainSpellCount=domain?domain.spells.filter(row=>level>=row[0]).length*2:0,intervention=level>=20?'Otomatik':level>=10?`d100 ≤ ${level}`:'Lv 10’da',core=window.V52_CLERIC_CORE_FEATURES||[];
