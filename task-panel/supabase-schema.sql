-- YENİ Supabase projesi için tam kurulum. Eski kurulum varsa hotel-roles-update.sql kullan.
create type public.app_role as enum ('admin','manager','reception','procurement','kitchen','housekeeping','maintenance','employee');
...
language sql stable security definer set search_path='' as $$
  select exists(select 1 from public.profiles where id=(select auth.uid()) and role::text in ('admin','manager'));
$$;
